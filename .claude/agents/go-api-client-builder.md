...
description: Use this agent when you need to build a production-grade HTTP API client in Go, particularly when integrating with external REST APIs that require authentication, rate limiting, retry logic, and robust error handling. This agent is especially valuable for API integrations like YNAB, Stripe, GitHub, or any third-party service where reliability and debuggability are critical.\n\nExamples:\n\n<example>\nContext: User is implementing a YNAB API integration and has just set up the project structure.\nuser: "I need to create a client for the YNAB API that handles authentication and rate limiting"\nassistant: "I'll use the go-api-client-builder agent to create a robust, production-ready YNAB API client with proper authentication, rate limiting, and error handling."\n<agent: go-api-client-builder>\n</example>\n\n<example>\nContext: User has written basic API client code but needs it enhanced with retry logic and error handling.\nuser: "Here's my basic YNAB client implementation. Can you make it production-ready with retries and better error handling?"\nassistant: "Let me use the go-api-client-builder agent to enhance this client with exponential backoff retry logic, comprehensive error handling, and production-grade reliability features."\n<agent: go-api-client-builder>\n</example>\n\n<example>\nContext: User is planning to integrate with multiple external APIs in their Go application.\nuser: "I'm building a financial aggregation service that needs to integrate with YNAB, Plaid, and Stripe APIs"\nassistant: "I'll use the go-api-client-builder agent to create robust HTTP clients for each of these APIs, ensuring consistent error handling, rate limiting, and retry logic across all integrations."\n<agent: go-api-client-builder>\n</example>
model: sonnet
color: green
---
